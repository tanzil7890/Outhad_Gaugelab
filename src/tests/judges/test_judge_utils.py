import pytest
from gaugelab.judges.utils import create_judge, InvalidJudgeModelError
from gaugelab.judges import LiteLLMJudge, MixtureOfJudges, TogetherJudge
from unittest.mock import patch


@pytest.fixture(autouse=True)
def mock_model_lists():
    with patch("gaugelab.judges.utils.LITELLM_SUPPORTED_MODELS", ["gpt-4"]):
        with patch(
            "gaugelab.judges.utils.TOGETHER_SUPPORTED_MODELS",
            [
...
def test_create_judge_invalid_type():
    ...
    with pytest.raises(
        ...
        match="Model must be a string, list of strings, or a gaugelab judge object",
    ):
