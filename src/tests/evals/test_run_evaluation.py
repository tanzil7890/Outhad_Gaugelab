...
from gaugelab.data import Example, ScoringResult, ScorerData, Trace
...
class TestRunEvaluation:
    @patch("gaugelab.run_evaluation.requests.post")
    ...
    async def test_get_evaluation_status(self):
        with patch("gaugelab.run_evaluation.requests.get") as mock_get:
    ...
    async def test_poll_evaluation_until_complete(self):
        with patch("gaugelab.run_evaluation.requests.get") as mock_get:
            ...
            with (
                patch("gaugelab.run_evaluation.requests.post") as mock_post,
                patch("asyncio.sleep"),
            ):
                mock_post.return_value.ok = True
