...
from gaugelab.common.tracer import Tracer, TraceManagerClient, TraceClient
...
def test_sync_graph_execution(project_name: str):
    """Tests synchronous graph execution with GaugelabCallbackHandler."""
    print("\n--- Running Sync Test ---")
    Tracer._instance = None
    tracer_sync = Tracer(
        api_key=API_KEY, organization_id=ORG_ID, project_name=project_name
    )
    handler_sync = GaugelabCallbackHandler(tracer_sync)
...
async def test_async_graph_execution(project_name: str):
    """Tests asynchronous graph execution with GaugelabCallbackHandler."""
    print("\n--- Running Async Test ---")
    Tracer._instance = None
    tracer_async = Tracer(
        api_key=API_KEY, organization_id=ORG_ID, project_name=project_name
    )
    handler_async = GaugelabCallbackHandler(tracer_async)
